		...
		switch (Systype)
			{
								...
								case SC_OpenFileID:
				{
					...
					if (fileSystem->Open(fileName, _type)!=NULL){
				...
				case SC_ReadFile:
				{
					...
					if(id<0||id>10||fileSystem->fileTable[id]==NULL)
					{
						machine->WriteRegister(2,-1);
						delete [] buf;
						break;
					...
					//Doc tu file
					int Pos1,Pos2;
					Pos1=fileSystem->fileTable[id]->GetCurrentPos();//vi tri dau file
					if(fileSystem->fileTable[id]->Read(buf,charCount)>0)
					{
						Pos2=fileSystem->fileTable[id]->GetCurrentPos();//vi tri cuoi file
						machine->System2User(buffer,Pos2-Pos1+1,buf);
				...
				case SC_WriteFile:
				{
					...
					buf = machine->User2System(buffer,charCount);
					//ghi file ra console
					if(fileSystem->fileTable[id]->_type==3)
					{
						int i=0;
						while(buf[i]!=0)
						{
					...
					int Pos1,Pos2;
					Pos1=fileSystem->fileTable[id]->GetCurrentPos();//vi tri dau file
					if(fileSystem->fileTable[id]->Write(buf,charCount)>0)
					{
						Pos2=fileSystem->fileTable[id]->GetCurrentPos();//vi tri cuoi file
                        machine->WriteRegister(2,Pos2-Pos1+1);
                        delete []buf;
				...
				case SC_Seek:
				{
					...
					if(id<0||id>10||fileSystem->fileTable[id]==NULL||pos<-1)
					{
						machine->WriteRegister(2,-1);
						break;
					}
					//seek tren console
					if(fileSystem->fileTable[id]->_type==2||fileSystem->fileTable[id]->_type==3)
					{
						machine->WriteRegister(2,-1);
						break;
					}
					...
					if(pos==-1)
					{
						int len = fileSystem->fileTable[id]->Length();
						fileSystem->fileTable[id]->Seek(len);
